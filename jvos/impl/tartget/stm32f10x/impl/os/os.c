...
{
    while ( xTaskIsTaskFinished( id ) != pdTRUE )
    {
        mos_msleep( 10 );
    }
}
...
uint32_t mos_eventgroup_set_bits(mos_eventgroup_id_t id, int bits)
{
    ...
    return xEventGroupSetBits(id, bits);
}
...
uint32_t mos_eventgroup_clear_bits(mos_eventgroup_id_t id, int bits)
{
    ...
    return xEventGroupClearBits(id, bits);
}
            ...
            int isClearOnExit, int isWaitForAllBits, int waitMs)
{
    ...
    return xEventGroupWaitBits( id, bits, isClearOnExit, isWaitForAllBits, waitMs );
}
